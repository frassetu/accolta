...
export async function POST(req: NextRequest) {
  ...
  if (contentType.includes('multipart/form-data')) {
    ...
    for (const row of allRows) {
      const key = `${row.artiste.toLowerCase()}|||${row.titre.toLowerCase()}`
      const existing = seen.get(key)
      if (!existing || (!existing.paroles && row.paroles)) seen.set(key, row)
    }
  ...
  if (action === 'upsert') {
    const { data, error } = await supabaseAdmin
      .from('chansons')
      .upsert(payload, { onConflict: 'artiste,titre' })
      .select()
