...
import { Plus, Pencil, Trash2, Upload, Search, ChevronLeft, Eye, EyeOff, LogOut, CheckCircle, AlertCircle, Loader, X } from 'lucide-react'
...
export default function AdminPanel({ isAdmin, onLogin, onClose }: Props) {
  ...
  const searchArtists = async (val: string) => {
    ...
    const { data } = await supabase
      .from('chansons').select('artiste').ilike('artiste', `${val}%`).limit(8)
    if (data) {
      const unique = [...new Set(data.map((r) => r.artiste))]
      setArtistSuggestions(unique)
    }
  }
  ...
  const searchAlbums = async (val: string, artiste: string) => {
    ...
    const { data } = await q
    if (data) {
      const unique = [...new Set(data.map((r) => r.album).filter(Boolean))]
      setAlbumSuggestions(unique)
    }
  }
  ...
  const handleSave = async () => {
    ...
    if (res.ok) {
      resetForm()
      if (!editSong) {
        // Rester sur le formulaire pour ajouter la suivante
        setTab('add')
      } else {
        setTab('songs')
      }
  ...
  if (!isAdmin) {
    return (
      <div className="min-h-screen bg-bg flex flex-col max-w-lg mx-auto">
        <button
          onClick={onClose}
          className="absolute top-14 left-4 w-9 h-9 rounded-xl bg-card flex items-center justify-center z-10"
        >
        ...
        {(['songs', 'add', 'import'] as AdminTab[]).map((t) => (
          <button
            ...
            onClick={() => {
              setTab(t)
              if (t !== 'add') resetForm()
              if (t === 'import') setImportStatus({ state: 'idle' })
            }}
            className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${
              tab === t ? 'bg-accent text-white' : 'bg-card text-text-muted'
            }`}
          >
            ...
            {loading ? (
              ...
              <div className="space-y-2 pb-10">
                  ...
                  <div key={song.id} className="flex items-center gap-3 p-3 rounded-xl bg-card">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-text-muted text-xs truncate">
                        {song.titre}{song.annee ? ` - ${song.annee}` : ''}
                      </p>
                    </div>
                    {!song.paroles && (
                      <span className="text-xs text-muted bg-border px-2 py-0.5 rounded-full flex-shrink-0">
                        Sans paroles
                      </span>
                    )}
                ...
                {filtered.length === 0 && !loading && (
                  <p className="text-center text-text-muted py-8">Aucune chanson</p>
                )}
              </div>
            )}
          </div>
        ...
        {tab === 'add' && (
          <div className="pb-10">
            <div className="flex items-center gap-3 mb-5">
              <button
                onClick={() => { setTab('songs'); resetForm() }}
              ...
              <h2 className="font-display font-semibold text-text">
                {editSong ? 'Modifier la chanson' : 'Nouvelle chanson'}
              </h2>
            </div>
            ...
            <div className="space-y-3">
                ...
                {showArtistSug && artistSuggestions.length > 0 && (
                  <div className="absolute z-20 w-full mt-1 bg-card border border-border rounded-xl overflow-hidden shadow-lg">
                    {artistSuggestions.map((a) => (
                      <button
                        key={a}
                        className="w-full text-left px-4 py-2.5 text-sm text-text hover:bg-border transition-colors"
                        onMouseDown={() => {
                          setForm((f) => ({ ...f, artiste: a }))
                          setShowArtistSug(false)
                          setArtistSuggestions([])
                        }}
                      >
                        {a}
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    searchAlbums(e.target.value, form.artiste)
                    setShowAlbumSug(true)
                  }}
              ...
              {/* N° piste */}
              <div>
                <label className="text-text-muted text-xs mb-1.5 block">N° de piste</label>
                <input
                  type="number"
                  inputMode="numeric"
                  pattern="[0-9]*"
            ...
            <button
              onClick={handleSave}
              disabled={!form.artiste.trim() || !form.titre.trim() || saving}
            ...
            {!editSong && (
              <p className="text-center text-text-muted text-xs mt-3">
                Le formulaire se videra apres ajout pour saisir la suivante
              </p>
            )}
          </div>
        )}
          ...
          <div className="space-y-4 pb-10">
            ...
            {importStatus.state === 'uploading' && (
              <div className="flex items-center gap-3 p-4 rounded-xl bg-card">
                ...
                <p className="text-text text-sm">Import en cours, merci de patienter...</p>
              </div>
            )}
            ...
            {importStatus.state === 'done' && (
              <div className="p-4 rounded-xl bg-green-500/10 border border-green-500/30 space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-green-400 font-semibold">Import termine !</p>
                </div>
                <div className="space-y-1 text-sm">
                  <p className="text-text-muted">Lignes dans le fichier : <span className="text-text font-medium">{importStatus.total_in_file}</span></p>
                  <p className="text-text-muted">Apres dedoublonnage : <span className="text-text font-medium">{importStatus.after_dedup}</span></p>
                  <p className="text-text-muted">Importees : <span className="text-green-400 font-medium">{importStatus.inserted}</span></p>
                </div>
                {importStatus.errors.length > 0 && importStatus.errors.map((e, i) => (
                  <p key={i} className="text-yellow-400 text-xs">{e}</p>
                ))}
                <button
